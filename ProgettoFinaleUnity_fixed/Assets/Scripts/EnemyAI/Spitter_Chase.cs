...
public class Spitter_Chase : Enemy_Chase
{
    ...
    SpitterSM sm;
    public Spitter_Chase(SpitterSM stateMachine) : base("Spitter_Chase", stateMachine)
    {
        sm = stateMachine;
    }
    ...
    {
        if (sm is SpitterSmallSM)
        {
            if (((SpitterSmallSM)sm).Leader.gameObject.activeSelf)
            {
                return true;
            }
            else
